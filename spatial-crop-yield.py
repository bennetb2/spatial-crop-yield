...
def filter_field_boundaries(cfg):
    ...
    for index, row in df.iterrows():
        try:
            # A "point" is the lat and long stored in a row of the geopandas "Point"
            ...
            for field_name in filtered_points:
                # If the point is within the field, and does not contain zeroes, add to that field
                # Repeated for every defined field
...
def delta_weight(cfg, filtered_points):
    ...
    for field in cfg['field_boundaries']:
        ...
        for i in range(0, len(filtered_points[field['name']])):
            ...
            i_point_series = geopandas.GeoSeries({'point': Point(float(filtered_points[field['name']][i]['latitude']), float(filtered_points[field['name']][i]['longitude']))})
            ...
            if distance > cfg['distance_interval']:
                try:
                    ...
                    # Removing anything that isn't a decimal or a digit from recorded weights
                    i_weight = float(re.sub('[^A-Za-z0-9.]+', '', filtered_points[field['name']][i]['weight']))
                    ...
                    if delta < 300 and delta > -500:
                        # Append statement calls for the lat/long of midpoint and delta weight per distance traveled (RATE) to be stored as a list
                        # Again, using structure of name-identified objects, filled w/ lists, all stored within an object (???)
                        harvest_yield[field['name']].append({'lat': midpoint.x['point'], 'long': midpoint.y['point'], 'delta_weight': delta/distance})

                    # Reset initial point for next run-through
                    initial_point = filtered_points[field['name']][i]
                except:
                    pass
        # Normalizing the delta weight by finding maximum value and dividing to create a percent/ratio
        max_weight = max([point['delta_weight'] for point in harvest_yield[field['name']]])
        for field_yield in harvest_yield[field['name']]:
            field_yield['delta_weight'] = field_yield['delta_weight']/max_weight

    # Final return should have an object that is a list of lists
    # harvest_yield is an object, filled with a list of objects, where each object is a field and those field objects contain lists of lat/long and delta weight
...
def main():
    with open("config.json", 'r') as json_data_file:
        ...
        output = delta_weight(cfg, filtered_points)
